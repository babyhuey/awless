	...
	for i, cmd := range cmdsReverseIterator {
		...
		if isRevertible(cmd) {
			...
			case "create":
				...
				case "application", "buildproject", "cachesubnetgroup", "backupvault", "configurationset", "crawler", "deployapplication", "emailidentity", "environment", "gluedatabase", "job", "rulegroup", "webacl", "dynamodbtable", "ekscluster", "configrule", "eventbus", "eventrule", "ipset", "loggroup", "redshiftsubnetgroup", "ssmparameter", "stream", "trail":
					params = append(params, fmt.Sprintf("name=%s", printItem(cmd.ParamNodes["name"])))
