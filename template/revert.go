	...
	for i, cmd := range cmdsReverseIterator {
		...
		if isRevertible(cmd) {
			...
			case "create":
				...
				case "application", "buildproject", "cachesubnetgroup", "configurationset", "crawler", "deployapplication", "emailidentity", "environment", "gluedatabase", "job", "rulegroup", "webacl", "dynamodbtable", "ekscluster", "configrule", "eventbus", "eventrule", "ipset", "loggroup", "redshiftsubnetgroup", "ssmparameter", "stream", "trail":
					params = append(params, fmt.Sprintf("name=%s", printItem(cmd.ParamNodes["name"])))
				case "deploymentgroup":
					params = append(params, fmt.Sprintf("name=%s", printItem(cmd.ParamNodes["name"])))
					params = append(params, fmt.Sprintf("application=%s", printItem(cmd.ParamNodes["application"])))
				case "eksnodegroup":
					params = append(params, fmt.Sprintf("name=%s", printItem(cmd.ParamNodes["name"])))
					params = append(params, fmt.Sprintf("cluster=%s", printItem(cmd.ParamNodes["cluster"])))
